...
if (!PIXI.TextureCache) {
    ...
    PIXI.Texture.fromFrame = function (frameId)
    {
        ...
        if (!texture)
        {
            texture = new Phaser.Image(game, 0, 0, frameId).texture;
        }
...
Phaser.Loader.prototype.EZGUITheme = function (key, url, callback, callbackContext) {
    return this.addToFileList('EZGUITheme', key, url);
}

Phaser.Loader.prototype.loadFile = (function() {
    var original_loadFile = Phaser.Loader.prototype.loadFile;

    return function(file){
        var _this = this;
        if (file.type == 'EZGUITheme')
        {
            EZGUI.Theme.load([file.url], function(){_this.fileComplete.call(_this, file);});
        }
        else
        {
            return original_loadFile.call(this, file);
        }
    }
})();